...
function roadTexture(renderer) {
  ...
  // divided-highway convention: yellow down the left edge, white down the right
  g.fillStyle = "#e0b530"; g.fillRect((SHOULDER + .2) * pxm, 0, .12 * pxm, 512);
    ...
    for (let k = kFrom; k <= kTo; k++) {
      ...
      if (c >= .5) {
        ...
        if (hash(k, 5, 5) < .05 && openK > .9) { // overhead sign gantry
          for (const px of [-12.6, 12.6]) B.pole.add(px, 0, z, .35, 8.5, .35);
          B.arm.add(0, 8.3, z, 25.6, .35, .35);
          B.sign.add(-4, 6.2, z, 7, 3, .25); B.sign.add(5, 6.2, z, 5, 3, .25);
        }
        if (biome === "desert") {
          for (let i = 0; i < 7 * openK; i++) {
            const side = r(i) < .5 ? 1 : -1;
            const x = side * (16 + r(i + 20) * 45);
          ...
          for (const side of [1, -1]) {
            ...
            while (zz < (k + 1) * SEG) {
              ...
              const x = side * (70 + r(side * 230 + i) * 30 + dep / 2);
          ...
          for (const side of [1, -1]) {
            ...
            B.hill.add(side * (110 + r(side * 310) * 60), -s * .55, z, s * 1.4, s, s, r(side * 320) < .5 ? 0x5e9a48 : 0x6fa852);
          }
        }
      }
    }
